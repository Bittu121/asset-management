...
export const getSupportGroupById = async (id: string) => {
  return await SupportGroup.findById(id).populate("manager", "name email employeeCode designation");
};
...
export const updateSupportGroup = async (id: string, data: UpdateSupportGroupDto) => {
  if (data.code) {
