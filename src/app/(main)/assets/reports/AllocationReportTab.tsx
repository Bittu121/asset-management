...
const DETAIL_HEADERS  = ["Asset Tag", "Asset Name", "Allocated To", "Allocation Date", "Expected Return", "Status"];
...
export default function AllocationReportTab() {
  ...
  // Filter allocations by status and search text
  const filteredAllocations = SAMPLE_ALLOCATIONS.filter((allocation) => {
    const matchesStatus = statusFilter === "ALL" || allocation.status === statusFilter;
    const searchText    = search.toLowerCase();
    const matchesSearch =
      allocation.assetTag.toLowerCase().includes(searchText) ||
      allocation.assetName.toLowerCase().includes(searchText) ||
      allocation.allocatedTo.toLowerCase().includes(searchText);
  ...
  function handleExport() {
    const rows = filteredAllocations.map((a) => [
      a.assetTag, a.assetName, a.allocatedTo,
      a.allocationDate, a.expectedReturn, a.status,
    ]);
    exportToCSV("allocation-report", rows, DETAIL_HEADERS);
  }

  // Build a per-user summary from all allocations
  const userNames   = Array.from(new Set(SAMPLE_ALLOCATIONS.map((a) => a.allocatedTo)));
  const userSummary = userNames.map((user) => {
    const userRows = SAMPLE_ALLOCATIONS.filter((a) => a.allocatedTo === user);
    return {
      user,
      active:   userRows.filter((a) => a.status === "ACTIVE").length,
      overdue:  userRows.filter((a) => a.status === "OVERDUE").length,
      returned: userRows.filter((a) => a.status === "RETURNED").length,
  ...
  const cardClass = "bg-white/80 backdrop-blur-sm border border-gray-100 rounded-3xl shadow-[0_8px_30px_rgba(15,23,42,0.04)]";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <StatCard
          label="Overdue"
          value={SAMPLE_ALLOCATIONS.filter((a) => a.status === "OVERDUE").length}
          color="bg-rose-50 text-rose-500"
          icon={<FiAlertTriangle size={22} />}
        />
        <StatCard
          label="Returned"
          value={SAMPLE_ALLOCATIONS.filter((a) => a.status === "RETURNED").length}
      ...
      <div className={cardClass}>
        <div className="p-6">
              ...
              <thead>
                <tr className="border-b border-gray-100">
                  {SUMMARY_HEADERS.map((header) => (
                    <th key={header} className="text-left py-3 px-4 text-xs font-semibold text-gray-400 uppercase">
                      {header}
                    </th>
                  ))}
                </tr>
          ...
          <div className="mt-5">
            <Pagination currentPage={summaryPage} totalPages={summaryTotalPages} onPageChange={setSummaryPage} />
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {paginatedDetail.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-10 text-center text-sm text-gray-400">No records found</td>
                  </tr>
                ) : (
                  paginatedDetail.map((a) => (
                    <tr key={a.id} className="border-t border-gray-50 hover:bg-gray-50/70">
                      ...
                      <td className="py-3 px-4"><Badge label={a.status} type={a.status} /></td>
                    </tr>
                  ))
                )}
              </tbody>
